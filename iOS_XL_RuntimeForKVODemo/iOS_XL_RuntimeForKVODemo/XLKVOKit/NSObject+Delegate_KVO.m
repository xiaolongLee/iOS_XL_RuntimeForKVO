...
static NSString * const kXLkvoClassPrefix = @"XLObserver_";
static NSString * const kXLkvoAssiocateObserver = @"XLAssiociateObserver";

@interface XL_ObserverInfo : NSObject
...
@implementation XL_ObserverInfo
...
static void KVO_setter(id self, SEL _cmd, id newValue) {
    ...
    //获取所有监听回调对象进行回调
    NSMutableArray *observers = objc_getAssociatedObject(self, (__bridge const void *)kXLkvoAssiocateObserver);
    for (XL_ObserverInfo *info in observers) {
        if ([info.key isEqualToString:getterName]) {
            dispatch_async(dispatch_queue_create(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                if ([info.observerDelegate respondsToSelector:@selector(XL_ObserverValueForKeyPath:ofObject:oldValue:newValue:)]) {
                    [info.observerDelegate XL_ObserverValueForKeyPath:getterName ofObject:self oldValue:oldValue newValue:newValue];
                }
            });
        }
    }
...
- (void)XL_addObserver:(NSObject *)observer forKey:(NSString *)key {
    ...
    if (![className hasPrefix:kXLkvoClassPrefix]) {
    ...
    XL_ObserverInfo *newInfo = [[XL_ObserverInfo alloc] initWithObserver:observer forKey:key];
    
    NSMutableArray *observers = objc_getAssociatedObject(self, (__bridge void *)kXLkvoAssiocateObserver);
    if (!observers) {
        observers = [NSMutableArray array];
        objc_setAssociatedObject(self, (__bridge void *)kXLkvoAssiocateObserver, observers, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
...
- (void)XL_removeObserver:(NSObject *)object forKey:(NSString *)key {
    NSMutableArray *observers = objc_getAssociatedObject(self, (__bridge void *)kXLkvoAssiocateObserver);
    XL_ObserverInfo *observerRemoved = nil;
    for (XL_ObserverInfo *observerInfo in observers) {
...
- (Class)createKVOClassWithOriginalClassName:(NSString *)className {
    NSString *kvoClassName = [kXLkvoClassPrefix stringByAppendingString:className];
